...
resource "google_service_account" "app" {
  account_id   = "${var.gcp_project_id}-app-sa"
  display_name = "${var.gcp_project_id} App Service Account"
  project      = var.gcp_project_id
  depends_on   = [var.required_apis]
}

resource "google_service_account" "worker" {
  account_id   = "${var.gcp_project_id}-worker-sa"
  display_name = "${var.gcp_project_id} Worker Service Account"
  project      = var.gcp_project_id
  depends_on   = [var.required_apis]
}
